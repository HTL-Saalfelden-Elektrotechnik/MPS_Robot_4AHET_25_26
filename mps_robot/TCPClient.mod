MODULE TCPClient
    ...
    FUNC bool sorting_start_belt()
        ...
        SocketCreate sorting_client_socket;
        SocketConnect sorting_client_socket, sorting_ip, sorting_port, \ Time := 3;
        ...
        IF ERRNO = ERR_SOCK_TIMEOUT THEN
            IF retry_no < 3 THEN
                ...
                RETRY;
            ELSE
                TPWrite "Sorting refusing connection, aborting";
                RETURN FALSE;
            ENDIF
        ENDIF
        
        TPWrite "sorting start belt";
